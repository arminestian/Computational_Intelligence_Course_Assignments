...
grid_size = 5
W = [random.uniform(0, 0.1)] * (grid_size * grid_size)
        ...
        for epoch in range(max_epochs):
            ...
            for label, inputs in dataset:
                ...
                for i in range(len(inputs)):
                    W[i] += alpha * error * inputs[i]
                b += alpha * error

                total_error += error ** 2

            if total_error < epsilon:
                result_label.configure(text=f"Training complete in {epoch+1} epochs.")
                return
...
train_button = ctk.CTkButton(app, text="Train Network", command=train_adaline)
